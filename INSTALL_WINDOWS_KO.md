# 연구 재현용 Windows 환경 준비 안내

> **이 문서는 정적 초기 PoC를 재현하기 위한 준비 안내입니다.**
> 일반적인 발견·조치 지연 문제에 대한 조건부 79 제안 중, 정답이 알려진 정지사진의
> 7·9 분류 단계만 시험합니다. 시민용 구조 앱이나 실제 CCTV 관제 시스템을 설치하는
> 문서가 아니며 신고·출동 기능도 설치하지 않습니다.

이 프로젝트는 공식 ZIP 또는 Git 소스 폴더에서 실행하는 공개 연구 패키지 형태만
안내합니다. PyPI 게시나 `pip install` wheel 설치는 지원하지 않습니다.

이 문서는 보호자, 교사, 전산 담당자 또는 컴퓨터 사용에 익숙한 사람을 위한 최초
환경 준비 안내입니다. 준비가 끝난 뒤 PoC 시험 참여자는
[초간단 재현법](QUICK_START_KO.md)의 네 단계만 따르면 됩니다.
...
GitHub 또는 GitLab의 공식 Release에서 Windows 재현용 ZIP을 받습니다. 출처를 알 수
없는 복사본은 사용하지 마세요.
...
PoC 시험 참여자에게는 다음 세 문서를 함께 전달하세요.
...
- [ ] 제3자 COCO 자세 가중치를 재현용 ZIP에 넣지 않았는지 확인
