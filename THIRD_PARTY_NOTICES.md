...
만화의 고양이, 인물, 장소, CCTV와 관제 장면은 설명을 위한 생성형 가상 표현이며
특정 사례·장소·기관·개인을 재현하지 않습니다. 사실관계의 증거나 제품
승인을 나타내지 않습니다. 그림 속 UI 캡처 부분은 프로젝트의 로컬 검토기와
생성형 정지사진 시험 결과를 설명하기 위해 사용됐습니다.

79는 국가표준이나 공인 긴급수신호가 아니라 제안·실험 단계의 사용자 정의 신호입니다.
`04_flow.png`의 시민→CCTV→AI 후보→관제요원 확인 흐름은 앞으로 검토하려는 목표
흐름이며 현재 공개한 정적 D7·D9 모델의 구현 기능이 아닙니다. 자동 신고·자동
출동은 포함하지 않습니다.

`06_validation.png`의 50/50은 사전 고정한 생성형 D7·D9 정지사진 50장에 한정된
결과입니다. 실제 CCTV 정확도, 비신호 행동에 대한 오경보율과 완전한 7979 연속동작
성능은 아직 측정되지 않았습니다.
